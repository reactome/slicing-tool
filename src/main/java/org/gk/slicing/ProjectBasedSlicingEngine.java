...
public class ProjectBasedSlicingEngine extends SlicingEngine {
    ...
    public void setUseforHierarchy(boolean useForHierarchy) {
        this.useForHierarchy = useForHierarchy;
    }
    ...
    public Map extractEvents() throws Exception {
        ...
        logger.info("Total events as specified: " + eventMap.size());
        if (useForHierarchy)
            extractEvents(events, 
                          regulations, 
                          eventMap, 
                          false);
        else if (useForSpecies)
            extractEvents(events, 
                          regulations, 
                          eventMap, 
                          true);
            ...
            extractEvents(events, 
                          regulations, 
                          eventMap);
        logger.info("Total instances after figuing out event references: " + eventMap.size());
        return eventMap;      
    }
